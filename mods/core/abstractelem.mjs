...
function substitute_baseURLs (url) {
  ...
  console.warn(
    'Probably an URL with a baseurl that has no match',
    url,
    window.YAMLbaseURLs
  )
  return url
...
class ElemLogic {
  retrieve_stored_URL (url) {
    if (url in window.loaded_URLs) return window.loaded_URLs[url]
    return undefined
  }
  ...
  load_obj (inp, unite_if_array = false) {
    ...
    if (
      Array.isArray(inp) &&
      unite_if_array /*inp.length && this.#is_yaml_url(inp[0])*/
    ) {
      let result = 0
      for (let i of inp) {
        ...
        if (Array.isArray(result)) result = result.concat(obj)
        else if (typeof result === 'object') Object.assign(result, obj)
        else result += obj
      }
      return result
    }
  ...
  #addDataAttributes (elem) {
    ...
    if (typeof obj === 'object' && !Array.isArray(obj))
      for (var key in obj) {
        var val = obj[key]
        var attr =
          'data-' + key.toLocaleLowerCase().replace(/[^a-zA-Z0-9]*/g, '') //https://www.w3schools.com/tags/att_global_data.asp
        if (
          typeof val === 'string' ||
          typeof val === 'number' ||
          typeof val === 'boolean'
  ...
  render_elem (type = 'stringify', details = null) {
    ...
    if ('attributes' in details)
      for (let k in details.attributes) {
        if (k.toLowerCase().substring(0, 2) == 'on') {
          console.debug(
            "attribute starting with 'on' can be a security risk",
            k
          )
          continue
        }
    ...
    if (this.elem) console.error('Somehow the elem was already bound')
    else this.elem = elem
    return elem
  }
  #generate_from_list () {
    let result = []
    for (let i of this.raw) result.push(this.#gen_list_item(i))
    return result
  }
  #gen_list_item (i) {
  ...
  get_elem () {
    if (typeof this.raw !== 'object') return this.render_elem('stringify')
    ...
    for (let elem of elems) cont.appendChild(elem)

    this.set_raw(cont)
    return cont
  }
  get_val (key) {
    if (typeof this.raw !== 'object') return this.raw
...
class AbstractElem extends ElemLogic {
  ...
  elem_details (key) {
    ...
    if (this.key)
      elems.container = {
        type: 'fieldset',
        attributes: this.default_attributes(), //{'class': this.key + ' ' + this.container_classname(key)},
        children: [
          {
            type: 'legend',
            innerText: this.key
...
class FlattenedElem extends AbstractElem {
  ...
  static flatten () {
    if (document.readyState !== 'complete')
      return window.alert(
        'Please wait until the page is fully loaded and try again'
      )
    const data = {
      url: window.dataURL,
